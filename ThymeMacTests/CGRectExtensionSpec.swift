  ...
  override func spec() {
    describe("CGRectExtension") {
      ...
      it("should return the top right point of the rect") {
        expect(rect.topRightPoint).to(equal(CGPoint(x: rect.width, y: rect.height)))
      }

      it("should return the bottom right point of the rect") {
        expect(rect.bottomRightPoint).to(equal(CGPoint(x: rect.width, y: 0)))
      }

      it("should return the bottom left point of the rect") {
        expect(rect.bottomLeftPoint).to(equal(CGPointZero))
      }
    }
  }
}
